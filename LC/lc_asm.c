...
/* if is legal reg, return 1; else return 0. */
int is_reg(char *reg) {
	int i = 0;
	while (regs[i] != NULL) {
...
/* if is legal reg, return the number of the reg. e.g. 31. */ 
...
/* if the imm_number = -0x1010, return 0; if imm_number = -a, return 1. */ 
...
/* 1<<base = 1*2^base.
 * if -(2^base) <= value <= 2^base -, return 1. */
...
int handle_label_imm(char *imm, TableEntry *pLabel, int cmdNum, int lineNum) {
	int val, ret;
	//if imm is -0x1010, return 0, !0 = 1 
	if (!(ret = is_imm(imm))) {
		// label_val get the address of the vlaue 
...
/* get the 5 bits in the beginning. */
...
/* if is label return VALID_LABEL. */
...
int label_val(char *labelName, int labelWid, TableEntry *pLabel, int cmdNum, int lineNum) {
	...
	while (*(tPtr->label) != '\0') {
		if (strncmp(tPtr->label, labelName, strlen(tPtr->label)) == 0 && \
			...
			offset <<= 2;
			//printf("The shifted offset is 0x%08x\n", offset);
			//printf("0x%08x\n", 1<<labelWid);
			//printf("offset >= 1<<labelWid is %d\n", (offset>= (1<<labelWid)));
			//printf("offset <= -1<<labelWid is %d\n", (offset < -(1<<labelWid)));
...
/* validate the instruction is in legal format. */
...
/*
 * remove '(' and ')'
 * return a PExpression structure, 
 * input: *pArg2, e.g. 8(t0)
 * output: ret->imm = 8 
 * 		   ret->reg = t0
 */
...
void handle_pseudo_cmd(FILE *file, int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,char *pArg3, TableEntry *pLabel) {
	if (!strncmp(pOpcode, "la", 2) && strlen(pOpcode) == 2) {
		/*
		 * Lab2-1 assignment
		 */
		//printf("Dealing with la. \n");
		fprintf(file,"0x%08x\n",cmd2code(lineNum, cmdNum, "lui", pArg1, "0x00", pArg3, pLabel));
		int add = label_val(pArg2, 31, pLabel, cmdNum, lineNum);
		//printf("0x%08x\n", add);
		char buffer[9];
		char address[11]= "0x";
		sprintf(buffer, "%08x", add);
	    strcat(address, buffer);
		//printf("The address is %s\n", address);
		fprintf(file,"0x%08x\n",cmd2code(lineNum, cmdNum, "addi", pArg1, pArg1, address, pLabel));
...
int cmd2code(int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,
	...
	if (!strncmp(pOpcode, "addi", 4) && strlen(pOpcode) == 4) {
		...
		code += (1 << 12);
		code += (reg2num(pArg2, lineNum) << 15);
		code += (lower5bit(pArg3, lineNum) << 20);
		...
		code = (0x0D << 2) + 0x03;
		//printf("first line for lui start.\n");
		code += (reg2num(pArg1, lineNum) << 7);	
		//printf("Second line for lui start. \n");
		code += (MASK11_0(validate_imm(pArg2, 20, lineNum)) << 12);
		//printf("Second line for lui finished. \n");
	...
	// Unconditional Jumps
		...
		code += (0x03) + (0x1b << 2);
		code += (reg2num(pArg1, lineNum) << 7);
		int offset = handle_label_imm(pArg2, pLabel, cmdNum, lineNum);
		// imm[20]
		code += ((offset & 0x100000) << 11);
		// imm[19:12]
		code += (offset & 0xFF000);
		// imm[11]
		code += ((offset & 0x800) << 9);
		// imm[10:1]
		code += ((offset & 0x7FF) << 20);
	...
	// Load and Store Instructions
		...
		validate_args(2, pArg1, pArg2, pArg3, lineNum);
		code += 3 + (1 << 12); 
		PExpression* ret = parse_parenthesized_expression(pArg1, lineNum);
		code += (reg2num(pArg2, lineNum) << 7);	
		code += (reg2num(ret->reg, lineNum) << 15);
		code += (ret->imm << 20);
		...
		validate_args(2, pArg1, pArg2, pArg3, lineNum);
		code += 3 + (2 << 12); 
		//printf("Start parse parenthesized. \n");
		PExpression* ret = parse_parenthesized_expression(pArg2, lineNum);
		//printf("Parse finsihed. \n");
		code += (reg2num(pArg1, lineNum) << 7);	
		//printf("first line finished. \n");
		code += (reg2num(ret->reg, lineNum) << 15);
		//printf("ret->reg finished.\n");
		code += (ret->imm << 20);
